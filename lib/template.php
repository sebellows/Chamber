...
class Template {
    ...
    public static function get_content_template() {
        ...
        if ( 'attachment' === $post_type ) {
            ...
            $type = Media::get_attachment_type();
        ...
        $templates[] = 'templates/content.php';
        $templates[] = 'templates/content/content.php';

        // Apply filters to the templates array.
        $templates = apply_filters( 'chamber/content/template/hierarchy', $templates );

        // Locate the template.
        $template = locate_template( $templates );

        // If template is found, include it.
        if ( apply_filters( 'chamber/content/template', $template, $templates ) )
            include( $template );
    }
    ...
    function get_embed_template() {
        ...
        if ( 'attachment' === $post_type ) {
            ...
            $templates[] = "templates/embed-attachment-{$type}.php";
            $templates[] = "templates/embed/attachment-{$type}.php";
        }
        ...
        if ( post_type_supports( $post_type, 'post-formats' ) ) {
            ...
            // Template based off post type and post format.
            $templates[] = "templates/embed-{$post_type}-{$post_format}.php";
            $templates[] = "templates/embed/{$post_type}-{$post_format}.php";

            // Template based off the post format.
            $templates[] = "templates/embed-{$post_format}.php";
            $templates[] = "templates/embed/{$post_format}.php";
        }

        // Template based off the post type.
        $templates[] = "templates/embed-{$post_type}.php";
        $templates[] = "templates/embed/{$post_type}.php";

        // Fallback 'embed/content.php' template.
        $templates[] = 'templates/embed/content.php';

        // Apply filters to the templates array.
        $templates = apply_filters( 'chamber/embed/template/hierarchy', $templates );

        // Locate the template.
        $template = locate_template( $templates );

        // If template is found, include it.
        if ( apply_filters( 'chamber/embed/template', $template, $templates ) )
            include( $template );
    }
    ...
    public static function get_header( $name = '' ) {
        ...
        $templates[] = 'templates/header.php';
